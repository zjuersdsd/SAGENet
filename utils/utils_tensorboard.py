...
def plot_rgb(batch_rgb_tensor, nb_display=4):
        '''
        ...
        for i in range(nb_display):
            rgb_image = axes[i].imshow(np.clip(batch_rgb_tensor[i].data.cpu().float().numpy().transpose(1, 2, 0), 0, 1))
            axes[i].set_title('RGB Image ' + str(i), size=8)
            axes[i].axis('off')
        plt.tight_layout()
        return figure

def tensorboard_display_input_pred(writer, input, pred, gt, mode, epoch):
    input_figure = plot_input(input['audio'])
    output_figure = plot_pred_gt(pred, gt)
    rgb_image = plot_rgb(input['img'])

    writer.add_figure(mode + '/' +  'Input Spec', input_figure, epoch)
    writer.add_figure(mode + '/' +  'Output Depth', output_figure, epoch)  
    writer.add_figure(mode + '/' + 'RGB image', rgb_image, epoch)